...
namespace Arg.DataAccess
{
    public class ClientSMTPAccountsImpl
    {
        ...
        {
            using var connection = Common.Database;
            var smtpAccount = connection.QueryFirstOrDefault<ClientSMTPAccounts>("GetSmtpAccountBySMTPAccountId", new { smtpAccountId }, commandType: CommandType.StoredProcedure);
            if (smtpAccount != null)
            {
                try
                {
        ...
        {
            const string query = @"DELETE FROM ClientSMTPAccounts 
                                   WHERE SMTPAccountId=@SMTPAccountId;";

            using var connection = Common.Database;
            var result = connection.Execute(query, new { smtpAccountId });
            return result;
        }
    }
}
