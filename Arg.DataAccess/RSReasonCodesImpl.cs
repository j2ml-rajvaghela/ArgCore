...
namespace Arg.DataAccess
{
    public class RSReasonCodesImpl
    {
        ...
        {
            const string query = @"DELETE FROM [ResearchItems.ReasonCodes] WHERE ReasonCodeId=@ReasonCodeId;";
            using (var connection = Common.Database)
            { 
                var result = connection.Execute(query, new { @ReasonCodeId = reasonCodeId});
                return result;
            }
        }
    }
